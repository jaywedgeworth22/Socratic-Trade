...
| Sign in with Apple native audience (`NATIVE_APPLE_CLIENT_ID`) | `trade.socratic.ios` | `src/lib/auth/apple-client-id.ts`. The native app audience MUST match the iOS bundle ID; mismatch produces a generic `Invalid callback URL` and breaks native auth. |
| APNs topic | `trade.socratic.ios` | `APNS_BUNDLE_ID` in prod Infisical; `src/lib/apns.ts` comment. The APNs HTTP/2 header `apns-topic` is exactly the bundle ID. |
...
| Server-side AASA `appIDs` claim | `CC8UTF7ATG.trade.socratic.ios` | `app/.well-known/apple-app-site-association/route.ts`. iOS fetches this on first launch and on associated-domains change. |
