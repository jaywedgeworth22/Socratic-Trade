> **2026-09-18 CURSOR — CI autofix: proposeTrades live-VIX fail-open (#3392).**  `proposeTrades` now prefers `fetchMacroDataWithLiveVix` but must not throw when that overlay is missing (`undefined` / reject).  Fall back to the 24h `fetchMacroData` snapshot, matching the vol-brake fail-open.  Extra-ship no.  No Coolify.  Do not merge #3392 from this tip.  Rollout: `docs/rollouts/2026-09-18-propose-trades-live-vix-fallback.md`.
