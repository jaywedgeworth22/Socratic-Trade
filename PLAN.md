> **2026-09-18 CURSOR — CI autofix: type `vixAsOf` on `MacroData` so `pruneMacro` can skip it.**  #3392 `7eb15e57` compared `key === "vixAsOf"` against `keyof MacroData` and CI tsc failed (TS2367).  Optional field on the interface; skip stays.  Extra-ship no.  No Coolify.  Rollout: `docs/rollouts/2026-09-18-prunemacro-vixasof-keyof.md`.
